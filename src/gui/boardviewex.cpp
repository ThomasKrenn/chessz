    ...
    ui(new Ui::BoardViewEx)
{
    ...
    // Load background image when creating the board
    if (AppSettings->getValue("/Board/Background").toBool())
    {
       QString path = AppSettings->getImagePath("background.jpg");
       bool isLoaded = _bkgnd.load(path);
    }
}
...
void BoardViewEx::paintEvent(QPaintEvent* pe)
{
   if (!_bkgnd.isNull()) {
      ...
      auto winSize = size();
      auto pixmapRatio = (float)_bkgnd.width() / _bkgnd.height();
      ...
      if (pixmapRatio > windowRatio)
      {
         ...
         painter.drawPixmap(offset, 0, newWidth, winSize.height(), _bkgnd);
      }
      else
      {
         auto newHeight = (int)(winSize.width() / pixmapRatio);
         auto offset = (newHeight - winSize.height()) / -2;
         painter.drawPixmap(0, offset, winSize.width(), newHeight, _bkgnd);
      }
   }
