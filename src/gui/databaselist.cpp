...
void DatabaseList::slotContextMenu(const QPoint& pos)
{
    ...
    if(m_cell.isValid())
    {
        ...
        menu.addSeparator();
#if defined(Q_OS_WIN)
        menu.addAction(tr("Show in Explorer"), this, SLOT(slotShowInFinder()))->setEnabled(bHasSuffix);
#elif defined(Q_OS_MACOS)
        menu.addAction(tr("Show in Finder"), this, SLOT(slotShowInFinder()))->setEnabled(bHasSuffix);
#else
        menu.addAction(tr("Show in File Manager"), this, SLOT(slotShowInFinder()))->setEnabled(bHasSuffix);
#endif
