    ...
    ui(new Ui::TagDetailWidget)
{
    ...
    // KNT Change to multi selection
    // This requires that the prev selection is deleted of games is deleted and a new selection with multiple ECO codes
    // is done from scratch.
    // 
    // ui->tagList->setSelectionBehavior(QAbstractItemView::SelectionBehavior::SelectRows);
    // ui->tagList->setSelectionMode(QAbstractItemView::SelectionMode::MultiSelection);
    // ui->tagList->setSelectionModel()
