    ...
    m_readAhead(0)
{
    ...
    addDockWidget(Qt::AllDockWidgetAreas, analysisDock);
    ...
    {        
        qWarning("new QTextToSpeech %p", speech);
...
MainWindow::~MainWindow()
{
    ...
    // Close databases
    QList<DatabaseInfo*> dbList = m_registry->databases();
    QListIterator<DatabaseInfo*> i(dbList);

    while (i.hasNext()) {
       DatabaseInfo* dbi = i.next();
...
// TODO Debug Tournament handles - they don't work reliably db.setTournament("");
    ...
    if (db.exec() == QDialog::Accepted)
    {
        ...
        if (tournament.isEmpty())
        {
            if (!account.isEmpty())
            {
               QDateTime startDT(start);
               QDateTime endDT(start);

               quint64 since = startDT.toMSecsSinceEpoch();
               quint64 until = endDT.toMSecsSinceEpoch();
...
void MainWindow::setupActions()
{
    ...
    /* Game->Load submenu */
    QAction * prevAction = createAction(tr("&Previous Game"), SLOT(slotGameLoadPrevious()), Qt::Key_F3,
                                        dbToolBar, ":/images/game_up.png");
    QAction * nextAction = createAction(tr("&Next Game"), SLOT(slotGameLoadNext()), Qt::Key_F4,
                                        dbToolBar, ":/images/game_down.png");
    QAction * goAction = createAction(tr("&Go to game..."), SLOT(slotGameLoadChosen()), Qt::CTRL | Qt::Key_G);
    QAction * rndAction = createAction(tr("&Random Game"), SLOT(slotGameLoadRandom()), Qt::CTRL | Qt::Key_Question,
...
void MainWindow::restoreRecentFiles()
{
    ...
    QStringList recentList = AppSettings->value("Files").toStringList();
    ...
    QStringListIterator recentIt(recentList);
    while (recentIt.hasNext()) {
       QString s = recentIt.next();
