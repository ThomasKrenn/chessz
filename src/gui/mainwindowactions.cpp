...
void MainWindow::slotFileNew()
{
    ...
    {
        MessageDialog::warning(tr("Cannot create ChessZ database."), tr("New database"));
    }
    else
    {
        pgnfile.close();
...
void MainWindow::updateWindowTitleFlipped(bool wasFlipped, bool m_flipped)
{
  ...
  if (!m_flipped){
    TitleFormatted = QStringList(
				 {whiteRook, " ", bullet, " ChessZ ", bullet, " %1"}
				 ).join("");
  }
  //Title for black pieces down uses black Rook
  if (m_flipped){
    TitleFormatted = QStringList(
				 {blackRook, " ", bullet, " ChessZ ", bullet, " %1"}
				 ).join("");
  }
...
BoardView* MainWindow::CreateBoardView()
{
    if (!databaseInfo()->IsBook())
    {
        BoardViewEx* frame = BoardViewFrame(m_boardView);
        if (frame) 
           frame->saveConfig(); // So that we can restore the layout below!
